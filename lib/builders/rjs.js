...
define([
	...
	var AMD_REGEXP = /(define)\s*\(\s*(?:\s*["']([^"']*)["']\s*,)?(?:\s*\[([^\]]*)\]\s*,)?/,
		...
		conduct = nodeRequire(require.toUrl('conduct')),

		traverse = recast.types.traverse,
		namedTypes = recast.types.namedTypes,
		builtInTypes = recast.types.builtInTypes,
	...
	function buildRoutes (routes) {
		var router = (new Function('conduct', 'return conduct(' + routes + ')')).call(null, conduct)
	...
	// 	def = recast.types.Type.def,
	// 	or = recast.types.Type.or;

	// def('RouteValueDescriptor')
	// 	.bases('ObjectExpression')
	// 	.build('descriptor', 'properties')
	// 	.field('properties', builtInTypes.string, function () {
	// 		return ''
	// 	})
	// 	.field('descriptor', or(namedTypes.Literal, namedTypes.Identifier) )

	// types.finalize()

	// var node = builders.routeValueDescriptor(builders.literal('pluto'))
	// inspect(node)
	// inspect(namedTypes.RouteValueDescriptor.check( node ))
	// inspect(printCode( node ))
